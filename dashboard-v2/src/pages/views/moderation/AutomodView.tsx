...
export default function AutomodView() {
  ...
  const { data, isLoading, error: automodError } = useQuery<AutomodConfig>({
  ...
  if (automodError) return <ErrorRetry message="Failed to load automod configuration" onRetry={() => window.location.reload()} />;
  if (isLoading || !data) return <LoadingFallback text="Loading automod..." />;
