...
function Requirements({ g, channels }: { g: Giveaway; channels: { id: string; name: string }[] }) {
  const parts = [];
  if (g.required_role_id) {
    const role = channels.find(c => c.id === g.required_role_id);
    parts.push(`@${role?.name || g.required_role_id}`);
  }
  if (g.required_level > 0) parts.push(`level ${g.required_level}+`);
  if (!parts.length) return <span className="text-[10px] text-muted-foreground">—</span>;
  return <Badge variant="outline" className="text-[9px]">{parts.join(" · ")}</Badge>;
}
...
export default function GiveawaysView() {
  ...
  const { data, isLoading, refetch, isFetching } = useQuery<GiveawaysData>({
    queryKey: ["giveaways", guildId, tab],
    queryFn: () => get(guildPath(`/api/giveaways?status=${tab}`, guildId)),
    enabled: !!guildId,
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Button-entry prize draws with optional role / level requirements. Start one with <span className="font-mono">/giveaway start</span>.</p>
          </div>
        </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList>
          <TabsTrigger value="active">Active ({data.status === "active" ? giveaways.length : "…"})</TabsTrigger>
          <TabsTrigger value="ended">Ended ({data.status === "ended" ? giveaways.length : "…"})</TabsTrigger>
        </TabsList>
                    ...
                    {giveaways.map(g => {
                      ...
                      return (
                        <TableRow key={g.id} className="border-b border-border/20">
                          ...
                          <TableCell className="text-xs"><Badge variant="outline" className="text-[10px]">{g.winners_count}</Badge></TableCell>
                          <TableCell className="text-xs text-muted-foreground">{g.entry_count}</TableCell>
                          <TableCell className="text-xs"><Requirements g={g} channels={data.channels} /></TableCell>
        ...
        <TabsContent value="ended" className="mt-3">
          <Card className="border-border/40 bg-card/40">
            <CardContent className="p-0">
              {giveaways.length === 0 ? (
                <div className="py-12 text-center text-sm text-muted-foreground">No ended giveaways yet.</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow className="border-b border-border/30">
                      ...
                      <TableHead className="text-xs">Winners</TableHead>
                      <TableHead className="text-xs">Entries</TableHead>
                      <TableHead className="text-xs">Ended At</TableHead>
                      <TableHead className="text-xs w-32"></TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                          ...
                          <div className="flex justify-end">
                            <Button size="sm" variant="ghost" disabled={rerollMutation.isPending} onClick={async () => {
                              if (!await confirm({
                                ...
                                description: `Draw new winners for "${g.prize}".`,
                                confirmLabel: "Reroll",
                              })) return;
